    ...
    >
      ...
      >
        <div className="flex flex-col items-center text-center gap-4">
          ...
          <h3 className="text-xl font-semibold text-[#262626]">Remove Team Member</h3>
          <p className="text-sm text-[#6B7280] leading-relaxed">
            Are you sure you want to remove <span className="font-semibold text-[#262626]">&quot;{member?.name}&quot;</span>? This action cannot be undone.
          </p>
...
// ─── Team Form Modal ────────────────────────────────────
function TeamFormModal({ member, onClose, onSave, saving }) {
  const [name, setName] = useState(member?.name || "");
  const [position, setPosition] = useState(member?.position || "Praxis Team");
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-5 border-b border-[#F0F0F0]">
          <div>
            ...
            <p className="text-xs text-[#9CA3AF] mt-0.5">
              {isEditing ? "Update the team member details below" : "Fill in the details to add a new team member"}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8">
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-col items-center">
              <label className="block w-full text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-2">
                Profile Image {!isEditing && <span className="text-red-400">*</span>}
              </label>
              <div
                onClick={() => fileInputRef.current?.click()}
                className="relative w-32 h-32 rounded-full border-2 border-dashed border-[#E5E7EB] overflow-hidden cursor-pointer hover:border-[#33187F]/30 transition-colors group"
              >
                {imagePreview ? (
            ...
            <div>
              <label className="block text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-2">
                Short Bio / Description <span className="text-red-400">*</span>
              </label>
              <textarea
                value={description}
        ...
        <div className="px-8 py-5 border-t border-[#F0F0F0] flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={saving || !name.trim() || (!isEditing && !imageFile)}
...
export default function AdminTeamPage() {
  ...
  // Fetch teams on mount and when search/page changes
  useEffect(() => {
    fetchTeams({ search, page, limit: 10 });
  }, [page]);
  ...
  const handleSearch = useCallback(
    (value) => {
      ...
      searchTimeout.current = setTimeout(() => {
        setPage(1);
        fetchTeams({ search: value, page: 1, limit: 10 });
      }, 400);
    },
    [fetchTeams]
  );
  ...
  const handleCreate = async (formData) => {
    ...
    if (result.success) {
      ...
      showToast(result.message || "Failed to add team member", "error");
    }
  };
  ...
  const handleUpdate = async (formData) => {
    ...
    if (result.success) {
      ...
      showToast(result.message || "Failed to update team member", "error");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    const result = await deleteTeamMember(deletingMember._id);
    setIsDeleting(false);
    if (result.success) {
      setDeletingMember(null);
      showToast("Team member removed successfully!");
    } else {
      showToast(result.message || "Failed to remove team member", "error");
    }
  };
  ...
  return (
    <div className="p-6 md:p-10 max-w-[1400px]">
      ...
      >
        <div>
          <h1 className="text-3xl font-medium text-[#262626] tracking-tight">Team Members</h1>
          <p className="text-sm text-[#9CA3AF] mt-1">
            Manage your team — {pagination?.total || 0} total members
          </p>
        </div>
        <motion.button
          whileTap={{ scale: 0.97 }}
      ...
      >
        {/* Table Header */}
        <div className="hidden md:grid grid-cols-[1fr_200px_400px_100px] gap-4 px-6 py-4 border-b border-[#F8F8F8] text-[10px] font-bold text-[#C0C0C0] uppercase tracking-[0.15em]">
          <span>Member</span>
          <span>Position</span>
        ...
        {loading && (
          <div className="flex items-center justify-center py-20">
            <div className="flex flex-col items-center gap-3">
              ...
              <span className="text-xs text-[#9CA3AF] font-medium">Loading team members...</span>
            </div>
          </div>
        )}
        ...
        {!loading &&
          teams.map((member, idx) => (
            <motion.div
              ...
              className="grid grid-cols-1 md:grid-cols-[1fr_200px_400px_100px] gap-3 md:gap-4 px-6 py-4 border-b border-[#F8F8F8] hover:bg-[#FCFCFC] transition-colors items-center group"
            >
              {/* Member Info */}
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[#262626] truncate">{member.name}</p>
                </div>
              </div>
              ...
              <div className="hidden md:block">
                <span className="text-xs font-medium text-[#4B5563] bg-[#F3F4F6] px-3 py-1.5 rounded-lg">
                  {member.position || "Praxis Team"}
                </span>
              </div>

              {/* Description */}
              <div className="hidden md:block text-xs text-[#6B7280] line-clamp-2">
                {member.description}
              </div>
